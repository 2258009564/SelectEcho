...
- 面板支持边界自适应，避免超出可视区域。
- 翻译中使用极简小胶囊，仅显示“翻译中...”，完成后自动展示完整结果。
- 翻译过程中按键/点击可随时打断，复制场景无需先手动关面板。
- 面板引擎支持下拉菜单点选，切换后可立即重译。
...
- 填写对应 `API Key`
- 长文本可开启流式输出，翻译期间会显示小胶囊“翻译中...”，完成后再展开结果
...
2. 松开鼠标后，选区附近出现翻译面板。
3. 翻译进行中会先显示小胶囊“翻译中...”。
4. 翻译中按任意键或点击任意位置可立即打断（滚轮不会打断）。
5. 翻译完成后会自动展示完整译文、源文（可选）和引擎标签。
6. 按 `Esc` 或点右上角 `×` 关闭面板。
7. 点击“复制”按钮可复制当前译文。
8. 需要调整引擎或 API 时，左键点击扩展图标可快速进入设置页。
9. 设置页右下角与翻译面板底部都提供了 GitHub Star 入口。
...
- 版本号升级为 `1.1.3`。
- 翻译进行中改为极简胶囊，只显示“翻译中...”，避免大窗口干扰。
- 翻译过程支持按键/点击即时打断（滚轮不打断），复制场景更顺畅。
- 翻译面板内引擎切换改为下拉菜单点选，切换后自动重译当前选区。
- 修复“点击打断后立即被重新触发翻译”的竞态问题。
